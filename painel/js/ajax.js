$(function(){
    $('.ajax').ajaxForm({
        ...
        success: function(data){
            ...
            $('.box-alert').remove();
            if(data.sucesso){
                $('.ajax').prepend('<div class="box-alert sucesso"><i class="fa-regular fa-circle-check"></i> '+data.mensagem+'</div>');
                if($('.ajax').attr('atualizar') == undefined)
                    $('.ajax')[0].reset();
            }else{
                $('.ajax').prepend( '<div class="box-alert erro"><i class="fa-solid fa-xmark"></i> '+data.mensagem+'</div>');
            }
